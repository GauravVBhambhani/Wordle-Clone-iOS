...
struct Guess {
    let index: Int // created when we create the object and it never changes
    var word = "     " // word is a string of 5 characters that's initially just 5 spaces and it's variable.
    var bgColors = [Color](repeating: .systemBackground, count: 5) // array of bg Color for each square initially set to the systems background color
